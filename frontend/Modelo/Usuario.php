...
WHERE
  ...
  public static function activarCuentaChangePass($id_usuario, $tipousuario, $email){
    if(empty($id_usuario) || empty($tipousuario)){return false;}
    if($tipousuario == 1){
             $sql = "UPDATE mfo_usuario_login SET password = '5725dbcc7254ee8422d1cb60db29625c' WHERE correo = '".$email."'";
             
             $GLOBALS['db']->Query($sql,array());
             return $GLOBALS['db']->update("mfo_usuario",array("estado"=>1),"id_usuario=".$id_usuario);
    }else{
      return $GLOBALS['db']->update("mfo_empresa",array("estado"=>1),"id_empresa=".$id_usuario);
    }
  ...
  public static function obtieneFoto($username){
    ...
    Utils::log("Ruta de la imagen: ".$rutaImagen);
    return $rutaImagen;   
  }

  public static function validaFoto($id_usuario){
    $sql = "SELECT * FROM mfo_usuario WHERE id_usuario=".$id_usuario;
    //Utils::log($GLOBALS['db']->auto_array($sql,array()));
    return $GLOBALS['db']->auto_array($sql,array());  
  }

  public static function validaFotoEmpresa($id_empresa){
    $sql = "SELECT * FROM mfo_empresa WHERE id_empresa=".$id_empresa;
    //Utils::log($GLOBALS['db']->auto_array($sql,array()));
    return $GLOBALS['db']->auto_array($sql,array());  
  }
  ...
  public static function obtenerAspirantes1($idOferta){
    ...
    $sql = "SELECT u.nombres, u.apellidos, u.foto, p.asp_salarial FROM mfo_usuario u inner join mfo_postulacion p ON u.id_usuario = p.id_usuario AND p.id_ofertas = ".$idOferta.";";
  ...
  public static function obtenerAspirantes($idOferta,$page,$limite,$cantd_faceta,$obtCantdRegistros=false){
    ...
    $subquery1 = "(SELECT o.id_ofertas, u.id_usuario,ul.username,u.nombres,u.apellidos,u.id_genero,u.foto,p.fecha_postulado,
    ...
    if($obtCantdRegistros == false){
      $sql .= "t2.id_ofertas, t2.id_usuario,t2.username,t2.nombres,t2.apellidos,t2.foto,t2.id_genero,t2.fecha_postulado, t2.fecha_nacimiento, t2.edad, t2.asp_salarial, e.descripcion AS estudios,t2.discapacidad,t2.id_situacionlaboral,t2.id_tipolicencia,n.nombre_abr AS nacionalidad, n.id_pais, pro.id_provincia, pro.nombre AS ubicacion,t2.pago, t1.test_realizados, t1.numero_test"; 
    }else{
  ...
  public static function filtrarAspirantes($idOferta,&$filtros,$page,$facetas,$limite,$usuarios_accesos,$obtCantdRegistros=false){
    $subquery1 = "(SELECT o.id_ofertas, u.id_usuario,ul.username,u.nombres,u.apellidos,u.foto,u.id_genero,p.fecha_postulado,u.id_situacionlaboral,u.id_tipolicencia, u.viajar, u.fecha_nacimiento,YEAR(NOW()) - YEAR(u.fecha_nacimiento) AS edad, p.asp_salarial,u.discapacidad,
  ...
  public static function busquedaGlobalAspirantes($id_pais_empresa,$cantd_faceta,$page,$obtCantdRegistros=false){     
    $sql = "SELECT ";
    $subquery1 = "(SELECT o.id_ofertas, u.id_usuario,ul.username,u.nombres,u.apellidos,u.id_genero,u.foto,u.fecha_creacion,
  ...
  public static function filtrarAspirantesGlobal($id_pais_empresa,&$filtros,$page,$facetas,$obtCantdRegistros=false){
    $subquery1 = "(SELECT u.id_usuario,ul.username,u.nombres,u.apellidos,u.foto,u.id_genero,u.fecha_creacion, u.id_situacionlaboral,u.id_tipolicencia, u.viajar, u.fecha_nacimiento,YEAR(NOW()) - YEAR(u.fecha_nacimiento) AS edad,u.discapacidad,
    u.id_escolaridad, u.id_nacionalidad,u.id_ciudad,IF(SUM(pl.costo) > 0 && up.estado = 1,1,0) AS pago 
    FROM
      mfo_usuario u, mfo_usuario_login ul, 
      mfo_oferta o,mfo_usuario_plan up,mfo_plan pl 
...
WHERE
  ...
  public static function listarCandidatosRegistrados(){
    $sql = "SELECT u.id_usuario, u.nombres, u.apellidos, u.estado, l.correo 
            FROM mfo_usuario u
            INNER JOIN mfo_usuario_login l ON l.id_usuario_login = u.id_usuario_login
            WHERE 1
  ...
  public static function listarCandidatosActivos(){
    $sql = "SELECT u.id_usuario, u.nombres, u.apellidos, u.estado, ul.correo
            FROM mfo_usuario u, mfo_usuario_login ul
            WHERE ul.id_usuario_login = u.id_usuario_login
            AND u.estado=1
            ORDER BY u.id_usuario";
    return $GLOBALS['db']->Query($sql,array());
  }
  ...
  public static function obtenerDatos($usuarios){
    ...
    $sql = 'SELECT u.id_usuario,u.nombres,u.apellidos,u.foto, ul.correo FROM mfo_usuario u, mfo_usuario_login ul WHERE u.id_usuario IN('.$usuarios.') AND u.id_usuario_login = ul.id_usuario_login';
