...
while( $rows = mysqli_fetch_array( $result_set, Database::ASSOC ) ){
  ...
  //buscar las subareas de la area ventas
  $subareasVentas = Array( 311, 312, 313, 314, 315, 316, 317, 318, 319, 320, 321 );

  $subAreas = explode(',', $str_areas);
    

  if( in_array(311, $subAreas) || in_array(312, $subAreas) || in_array(313, $subAreas) || in_array(314, $subAreas) || in_array(315, $subAreas) || in_array(316, $subAreas) || in_array(317, $subAreas) || in_array(318, $subAreas) || in_array(319, $subAreas) || in_array(320, $subAreas) || in_array(321, $subAreas) ){
    $arrayAreas = array_merge( $subAreas , $subareasVentas );
    $arrayAreas = array_unique($arrayAreas);
    $arrayAreas = implode($arrayAreas,',');
    $str_areas = $arrayAreas;
    echo '<br>'.$rows["nombres"].' entro aqui<br>';
  }


  //echo 'Nuevas areas '.$arrayAreas;
  ...
  foreach($arr_planes as $plan){
    ...
    //No se tomara en cuenta si el usuario es del area de ventas, pedido por Dorita 24/10/2019
    if( in_array(311, $subAreas) || in_array(312, $subAreas) || in_array(313, $subAreas) || in_array(314, $subAreas) || in_array(315, $subAreas) || in_array(316, $subAreas) || in_array(317, $subAreas) || in_array(318, $subAreas) || in_array(319, $subAreas) || in_array(320, $subAreas) || in_array(321, $subAreas) ){
      $flag_provincia = 0;
      $flag_ciudad = 0;
      echo '<br>'.$rows["nombres"].' queda sin ciudad y sin provincia<br>';
    }
    ...
    $arr_ofertas = Modelo_Oferta::obtieneAutopostulaciones($rows["id_pais"],$fechacalculada,$str_areas,
                                                           ...
                                                           Utils::log("-----------------".print_r($arr_ofertas , true));  
  ...
  if (!empty($mail_ofertas)){
    ...
    //Utils::envioCorreo('desarrollo@micamello.com.ec',"Postulaciones Automáticas",$email_body);   
  }
