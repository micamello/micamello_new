...
/*Script que consultara diariamente las ofertas aprobadas para envio de correos a todos los candidatos activos*/
...
$result_set = Modelo_Usuario::listarCandidatosActivos();
...
//   // var_dump($rows);
//   echo "Usuario ". $rows['id_usuario'].":".$rows['nombres'].' '.$rows['apellidos']." <br>";
  ...
  //echo "correos enviados: ".$correosEnviados."<br>Id_usuario ultimo usuario:".$rows['id_usuario']." <br>";
  ...
  if($correosEnviados >= 1000 || $minutosDiff >= 60)
  {
    if($ultimousuariofile = file_exists(CRON_RUTA.'ultimousuariosinfiltro.txt'))
    {
      unlink(CRON_RUTA.'ultimousuariosinfiltro.txt');
    }
    Utils::crearArchivo(CRON_RUTA, 'ultimousuariosinfiltro.txt', $rows["id_usuario"]);
    break;
  }
  else
  {
    ...
    if($ultimousuariofile = file_exists(CRON_RUTA.'ultimousuariosinfiltro.txt'))
    {
      $file = fopen("ultimousuariosinfiltro.txt", "r");
    ...
    // echo "areas: ".$str_areas."<br>"; 
       ...
       $ofertas = Modelo_Oferta::ofertasDiariasSinFiltro(1, $rows);
    
    //echo $ofertas;
    
    //echo 'email enviado a : '.$rows["correo"];
...
//     $sucursal = Modelo_Sucursal::consultaxPais($rows["id_pais"]);
    ...
    { 
       ...
       //echo $oferta["titulo"] . ' ' . $oferta["id_ofertas"].'<br>';   
      ...
      $mail_ofertas .= $nombre_empresa."<br>";
...
echo "correos enviados: ".$correosEnviados;
// //elimina archivo de procesamiento
// // unlink(CRON_RUTA."ultimousuario.txt");
 //unlink(CRON_RUTA.'alerta_ofertas_sin_filtro.txt');
?>